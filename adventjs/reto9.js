...
function countTime(leds) {
  let sum = leds.reduce((sum, item) => sum + item, 0);
  ...
  while (swich) {
    if (sum === leds.length) break;

    res = leds.map((item, index, array) => {
      if (item === 0) {
        if (index === 0) {
          return array[array.length - 1] === 1 ? 1 : 0;
        }
        return array[index - 1] === 1 ? 1 : 0;
      }
    ...
    sum = res.reduce((sum, item) => sum + item, 0);
